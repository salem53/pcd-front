...
@Component({
  selector: 'app-update-client-data-profile',
  templateUrl: './update-client-data-profile.component.html',
  styleUrls: ['./update-client-data-profile.component.css']
})
export class UpdateClientDataProfileComponent implements OnInit {
  ...
  constructor(public service : ClientService,public router : Router) {
    this.service.getClientByEmail(sessionStorage.getItem("username"))
      .subscribe(response => {
          this.id = response['id'];
  ...
  updateFreelancer(myform) {
    ...
    this.service.updateClientProfile(this.freelancer).subscribe(
      response => {
